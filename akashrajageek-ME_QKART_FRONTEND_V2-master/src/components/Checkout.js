...
const Checkout = () => {
  ...
  const handleProductFetchError = (error) => {
    if (error.response) {
      if (error.response.status === 500) {
        enqueueSnackbar(error.response.data.message, { variant: "error" });
      } else {
        enqueueSnackbar("An error occurred while fetching products.", {
          variant: "error",
        });
      }
    } else if (error.request) {
      enqueueSnackbar("Unable to reach the server. Please check your internet connection.", {
        variant: "error",
      });
    } else {
  ...
  //end of the fetch of entire product
  ...
  const fetchCart = async (token) => {
    ...
    try {
      ...
      handleCartFetchError(error);
      return null;
    }
  };
  
  const handleCartFetchError = (error) => {
    enqueueSnackbar("An error occurred while fetching cart details.", {
      variant: "error",
    });
   ...
   */
   const deleteAddress = async (token, addressId) => {
    try {
      const response = await deleteAddressOnBackend(token, addressId);
      handleDeleteAddressSuccess(response);
    } catch (error) {
      handleDeleteAddressError(error);
    }
  };
  
  const deleteAddressOnBackend = async (token, addressId) => {
    return axios.delete(`${config.endpoint}/user/addresses/${addressId}`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
  ...
  const handleDeleteAddressSuccess = (response) => {
    setAddresses({ ...addresses, all: response.data });
    enqueueSnackbar("Address deleted successfully", { variant: "success" });
  };
  
  const handleDeleteAddressError = (error) => {
    if (error.response) {
      enqueueSnackbar(error.response.data.message, { variant: "error" });
    } else {
      enqueueSnackbar(
